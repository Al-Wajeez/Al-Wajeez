...
let options = {
  ...
  jsPDF: {
    ...
    // Convert the canvas to a data URL and create a PDF
    let pageWidth = html2pdf.internal.pageSize.width;  // Corrected property access
    let pageHeight = html2pdf.internal.pageSize.height; // Corrected property access
    let imgWidth = pageWidth;
    let imgHeight = (canvas.height * imgWidth) / canvas.width;

    html2pdf.addImage(options, 'JPEG', 0, 0, imgWidth, imgHeight);

    // Download the PDF
    //pdf.save('Udemy-certificate.pdf');
    html2pdf().set(options).from(certif).save()
